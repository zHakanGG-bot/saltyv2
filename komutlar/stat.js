...
exports.run = async(client, message, args) => {
 if (!message.member.hasPermission('ADMINISTRATOR')) return message.channel.send('Bu komut için yeterli iznin **yok!**')
  ...
  if (args[0] == 'kur') {
if (db.has(`üyekanal_${message.guild.id}`)) return message.channel.send('‼️ Zaten istatistik **kurulu!**')
if (db.has(`kulkanal_${message.guild.id}`)) return message.channel.send('‼️ Zaten istatistik **kurulu!**')
